...
class modelEstadia extends DB{
   ...
   public function __construct()
   {
    ...
    public function getEstadiasWithClientNames() {
            ...
            $sql = $this->db->connect()->prepare( 'SELECT estadia.*, clientes.nombre
            FROM estadia 
    ...
    public function insertEstadia($idUnidad,$idEstacionamiento,$fechaInicio,$fechaFin, $idCliente){
            ...
            $sql = $this->db->connect()->prepare('INSERT INTO estadia (id_unidad,idEstacionamiento ,fechaInicio, FechaFin,en_curso, id_Cliente ) VALUES (?,?,?,?,?,?)');
    ...
    function updateEstadia($idUnidad, $idEstacionamiento, $fechaInicio, $FechaFin, $id_Cliente,$id){
        $query= $this->db->connect()->prepare('UPDATE estadia SET id_unidad=?,idEstacionamiento=?,fechaInicio=?,FechaFin=?,id_Cliente=? WHERE Id_estadia = ?');
